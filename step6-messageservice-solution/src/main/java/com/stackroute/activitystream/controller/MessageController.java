...
public class MessageController {
	...
	public ResponseEntity<Message> createUser(@PathVariable("circleName") String circleName,
	...
	public ResponseEntity<Message> sendMessageToUser(@PathVariable("receiverId") String receiverId,
		...
		try {
			...
			boolean result = messageDAO.sendMessageToUser(receiverId, message);
	...
	public ResponseEntity<List<Message>> getMessagesByCircle(@PathVariable("circleName") String circleName,
		...
		try {
			...
			List<Message> messages = messageDAO.getMessagesFromCircle(circleName, pageNumber);
	...
	public ResponseEntity<List<String>> listAllTags(HttpSession session) {
		try {
			...
			List<String> listAllTags = messageDAO.listTags();
			return new ResponseEntity<List<String>>(listAllTags, HttpStatus.OK);
		} catch (Exception e) {
	...
	public ResponseEntity<List<Message>> showMessagesWithTag(@PathVariable("tag") String tag,
		...
		try {
			...
			List<Message> messages = messageDAO.showMessagesWithTag(tag, pageNumber);
	...
	public ResponseEntity<UserTag> subscribe(@PathVariable("username") String username, @PathVariable("tag") String tag,
			HttpSession session) {
		try {
			
			boolean result = messageDAO.subscribeUserToTag(username, tag);
			if (!result)
				throw new Exception();
	...
	public ResponseEntity<UserTag> unsubscribe(@PathVariable("username") String username,
			@PathVariable("tag") String tag, HttpSession session) {
		try {
			
			boolean result = messageDAO.unsubscribeUserToTag(username, tag);
			if (!result)
				throw new Exception();
			return new ResponseEntity<UserTag>(HttpStatus.OK);
		} catch (Exception e) {
	...
	public ResponseEntity<List<String>> getTagSearchUser(@PathVariable("username") String username,
			HttpSession session) {
		try {
			
			List<String> messages = messageDAO.listMyTags(username);
